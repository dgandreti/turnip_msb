...
function addConsultantValidate() {
    ...
    if (relocation == 'Yes')
    {
        ...
        if (preferRegion == "-1")
        {
            $("addCnsltError").html(" <font color='red'>Prefer Region is Required</font>");
...
function AddConsultantStateChange()
{
    ...
    if (id == -1) {
      ...
      if (regionId == -1) {
        $("addCnsltError").html(" <font color=#FF4D4D>Preffered Region field is Required</font>.");
        $("#consult_preferredRegion").css("border", "1px solid red");
    }
    else {
    ...
    var url = CONTENXT_PATH + '/recruitment/consultant/getStateBasedOnRegion.action?id=' + id+'&regionId='+regionId;
    var req = initRequest(url);
    req.onreadystatechange = function() {
        if (req.readyState == 4 && req.status == 200) {
...
function AddConsultantSkillChange(){
  ...
  //  alert("priSkillId----->"+priSkillId)
    ...
    if ($("div").is("#skillCategoryValue"))
    {
        ...
        $(".skills").after('<select id="skillCategoryValue" class=" "  onclick="" multiple="multiple" tabindex="10" name="skillCategoryValue "><option value="-1">Select Skills</option></select>');
    }
    var url = CONTENXT_PATH + '/recruitment/consultant/getSkillsBasedOnPrimarySkill.action?priSkillId=' + priSkillId; 
   // alert("url--->"+url)
    var req = initRequest(url);
    req.onreadystatechange = function() {
        if (req.readyState == 4 && req.status == 200) {
